...
def convert_protocol_from_indiv_prot_xml_to_json_C(topo_lookup_tbl_json_path,
    ...
    for i, target_prot_name in enumerate(target_prot_name_list):
        ...
        for i, entry_xml in enumerate(found_entry_xml_list):
            ...
            elif "monitoring" in entry_xml.findtext(".//MlModeScanType/RangeName").lower() or "test bolus" in entry_xml.findtext(".//MlModeScanType/RangeName").lower():
...
target_prot_xml_path_list = []
for i, target_prot_name in enumerate(target_prot_name_list):
    target_prot_file_name = re.sub(r" \((Child|Adult)\)", r".\1", target_prot)
