    ...
    for l in recon_per_scan_list:
        ...
        for i in range(se_processed_winner, se_processed_winner+l[winner_index]+1):
            ...
            if i >= se_processed_winner:
                ...
                elif ('Topogram' in " ".join([se[para] for para in se.keys()])) or ('PreMonitoring' in " ".join([se[para] for para in se.keys()])) or ('Monitoring' in " ".join([se[para] for para in se.keys()])):
                    continue
...
for i, target_prot_name in enumerate(target_prot_name_list):
    ...
    target_prot_xml_path_list.append(glob.glob(protocol_file_folder + "\\**\\" + target_prot, recursive=True)[0])
