...
#target_prot_name_list = 'DE_ABDOMEN_OVER_55KG_CLASS_1.Adult'
#target_prot_xml_path_list = 
    

def convert_protocol_from_indiv_prot_xml_to_json(topo_lookup_tbl_json_path,
                                                 scan_a_lookup_tbl_json_path,
                                                 scan_b_lookup_tbl_json_path,
    ...
    print("loaded scan_a_lookup_tbl from ./para_lookup_tbl/scan_a_lookup_tbl.json")
    with open(scan_a_lookup_tbl_json_path, 'r') as fp:
        scan_a_lookup_tbl = json.load(fp)
    
    print("loaded scan_b_lookup_tbl from ./para_lookup_tbl/scan_b_lookup_tbl.json")
    with open(scan_b_lookup_tbl_json_path, 'r') as fp:
        scan_b_lookup_tbl = json.load(fp)
    ...
    for i, target_prot_name in enumerate(target_prot_name_list):
        ...
        for i, entry_xml in enumerate(found_entry_xml_list):
            ...
            if entry_xml.findtext(".//MlModeScanType/RangeName") == '"Topogram"':
                ...
                for key in scan_a_lookup_tbl:
                    target_func_name = scan_a_lookup_tbl[key][0]
                    para_dict = scan_a_lookup_tbl[key][1]
                    para = globals()[target_func_name](prot_xml_tree,
                                                       entry_node_id_value=entry_node_id_value,
                                                       bool_verbose=False,
                                                       **para_dict)
                ...
                dict3={}
                for key in scan_b_lookup_tbl:
                    target_func_name = scan_b_lookup_tbl[key][0]
                    para_dict = scan_b_lookup_tbl[key][1]
                    para = globals()[target_func_name](prot_xml_tree,
                                                       entry_node_id_value=entry_node_id_value,
                                                       bool_verbose=False,
                                                       **para_dict)
                ...
                scan_se = pd.Series(dict3)
                print(scan_se)
                prot_se_list.append(scan_se)
                prot_dict_list.append(dict3)
                scan_se = pd.Series(dict1)
                print(scan_se)
                prot_se_list.append(scan_se)
                prot_dict_list.append(dict1)
...
scan_a_lookup_tbl_json_path = "./para_lookup_tbl/scan_a_lookup_tbl.json"
scan_b_lookup_tbl_json_path = "./para_lookup_tbl/scan_b_lookup_tbl.json"
...
target_prot_name_list = ["DE_CTA_BIPHASE_LIVER_OVER_55KGAdult"]
common_prot_name = "BiphaseLiverOver"
...
for i, target_prot_name in enumerate(target_prot_name_list):
    ...
    protocol_file_folder = "G:\\My Drive\\BCH\\Rotations\\Informatics\\CT wiki\\Siemens_Force\\Import_adult\\DualEnergy"
